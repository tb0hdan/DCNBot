...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Command-line interface for the Meshtastic-Telegram Gateway.")
    subparsers = parser.add_subparsers(dest='command', required=True)

    # --- NEW: Sub-parser for the 'generate-id' command ---
    generate_parser = subparsers.add_parser('generate-id', help='Generate a new random hex ID for a node.')

    # Sub-parser for the 'nodes' command
    nodes_parser = subparsers.add_parser('nodes', help='List all known nodes from the database.')
    ...
    dm_parser.add_argument('node', help='The name or hex ID (e.g., !a1b2c3d4) of the destination node.')
    dm_parser.add_argument('message', nargs='+', help='The message to send.')

    args = parser.parse_args()

    # --- NEW: Handle the 'generate-id' command ---
    ...
    try:
        if args.command == 'nodes':
            ...
            for node in nodes:
                ...
                print(f"{node_id_hex:<12} {str(long_name or ''):<25} {str(short_name or ''):<12} {last_heard_str}")
            ...
            if args.command == 'dm':
                ...
                if node_identifier.startswith('!'):
                    ...
                    destination_id = db.get_node_id_by_name(node_identifier)
